...
/**
 * Central GUI texture atlas enum — every Destroy-mod GUI screen (Seismograph, Keypunch, Vat,
 * Redstone Programmer, Custom Explosive, Blowpipe, etc.) pulls its texture coordinates from here.
 * Each enum value captures one texture or one sub-region of a texture atlas PNG.
*/
public enum DestroyGuiTextures implements IGuiTexture {
    ...
    // Annotation dropped in the 1.21 port; constants preserved in-place until the library pushes
    // matching IGuiTexture entries.
    ...
    DestroyGuiTextures(String location, int width, int height) {
        this(location, 0, 0, width, height);
    }

    DestroyGuiTextures(String location, int startX, int startY, int width, int height) {
        this(location, startX, startY, width, height, 256, 256);
    }

    DestroyGuiTextures(String location, int startX, int startY, int width, int height, int textureWidth, int textureHeight) {
    ...
    private static final DestroyGuiTextures[] notes = new DestroyGuiTextures[] {
        REDSTONE_PROGRAMMER_NOTE_0, REDSTONE_PROGRAMMER_NOTE_1, REDSTONE_PROGRAMMER_NOTE_2, REDSTONE_PROGRAMMER_NOTE_3,
        REDSTONE_PROGRAMMER_NOTE_4, REDSTONE_PROGRAMMER_NOTE_5, REDSTONE_PROGRAMMER_NOTE_6, REDSTONE_PROGRAMMER_NOTE_7,
        REDSTONE_PROGRAMMER_NOTE_8, REDSTONE_PROGRAMMER_NOTE_9, REDSTONE_PROGRAMMER_NOTE_10, REDSTONE_PROGRAMMER_NOTE_11,
        REDSTONE_PROGRAMMER_NOTE_12, REDSTONE_PROGRAMMER_NOTE_13, REDSTONE_PROGRAMMER_NOTE_14, REDSTONE_PROGRAMMER_NOTE_15
    };
    ...
    /**
 * Custom 3D-space render (used by {@code SeismographItemRenderer} for one-handed held-map
 * overlay). 1.21 BufferBuilder pipeline.
*/
    @OnlyIn(Dist.CLIENT)
    public void render(PoseStack ms, float x, float y) {
        ...
        bufferbuilder.addVertex(matrix, x, y, 0f).setUv(uvx1, uvy1);
        bufferbuilder.addVertex(matrix, x, y + height, 0f).setUv(uvx1, uvy2);
        bufferbuilder.addVertex(matrix, x + width, y + height, 0f).setUv(uvx2, uvy2);
        bufferbuilder.addVertex(matrix, x + width, y, 0f).setUv(uvx2, uvy1);
