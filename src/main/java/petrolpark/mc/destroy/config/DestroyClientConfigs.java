...
public class DestroyClientConfigs extends DestroyConfigBase {
    ...
    public static int getLeftSlots(int extraSlots) {
        ...
        switch (DestroyConfigs.client().extraHotbarSlotLocation.get()) {
            ...
            case PRIORITY_LEFT: return Math.min(extraSlots, DestroyConfigs.client().extraHotbarPrioritySlotCount.get());
            case PRIORITY_RIGHT: return Math.max(0, extraSlots - DestroyConfigs.client().extraHotbarPrioritySlotCount.get());
        }
        return 0;
    }
