...
public class DestroySubstancesConfigs extends DestroyConfigBase {
    ...
    public final ConfigInt quicklimeBaseDecayTime = i(12000, 1, Integer.MAX_VALUE, "quicklimeBaseDecayTime", "The base number of ticks it takes for Quicklime to turn into Calcium Carbonate");
    ...
    public final EnumMap<SoundSource, ConfigFloat> soundSourceThresholds = enumFloatMap(SoundSource.class, SoundSource.values(), v -> "hangoverDamage" + pascal(v.name()) + "Threshold", v -> new String[]{"The volume threshold for sounds from the source " + pascal(v.name()) + " to damage entities with hangovers", "[1 will disable damage for that sound source]"}, 0f, 1f, 1f, 0.5f, 1f, 0.5f, 0.5f, 0.5f, 0.5f);
    public final EnumMap<SoundSource, ConfigFloat> soundSourceDamage = enumFloatMap(SoundSource.class, SoundSource.values(), v -> "hangoverDamage" + pascal(v.name()), v -> new String[]{"The damage done by sounds from the source " + pascal(v.name()) + " to entities with hangovers"}, 0f, Float.MAX_VALUE, 1f);
