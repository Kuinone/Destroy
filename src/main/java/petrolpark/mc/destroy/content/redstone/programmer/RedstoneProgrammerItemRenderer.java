...
/**
 * Custom item renderer for the Redstone Programmer BlockItem — reuses the BER's cylinder +
 * needle + transmitter partials, but with an important twist: transmitters only render if the
 * held stack's program has channels. "Running" animation only activates when the item is in a
 * hand display context AND the program isn't paused.
*/
public class RedstoneProgrammerItemRenderer extends CustomRenderedItemModelRenderer {
    ...
    protected void render(ItemStack stack, CustomRenderedItemModel model, PartialItemModelRenderer renderer,
        ...
        boolean running = transformType != ItemDisplayContext.GROUND
            && transformType != ItemDisplayContext.FIXED
            && programOptional.map(program -> !program.paused).orElse(false);
