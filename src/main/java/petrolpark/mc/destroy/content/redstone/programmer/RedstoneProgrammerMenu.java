...
/**
 * Container Menu for the Redstone Programmer's channel sequencer UI. Hosts a dynamic
 * {@link ItemStackHandler} "ghost inventory" — each channel gets 2 slots (First + Last frequency
 * items). Player-side: sets a frequency item → triggers {@link #sync} that broadcasts a
 * {@link RedstoneProgramSyncC2SPacket} to server so the server-side {@link RedstoneProgram} is
...
public class RedstoneProgrammerMenu extends GhostItemMenu<RedstoneProgram> {
    ...
    public void refreshSlots(int offset) {
        ...
        for (int channel = 0; channel < Math.min(contentHolder.getChannels().size() + 1,
                DestroyAllConfigs.SERVER.blocks.redstoneProgrammerMaxChannels.get()); channel++) {
    ...
    public void sync() {
        CatnipServices.NETWORK.sendToServer(new RedstoneProgramSyncC2SPacket(contentHolder));
    }

    @Override
    protected void saveData(RedstoneProgram contentHolder) {
        // Nothing to save externally — contentHolder is the authoritative source
    }

    public static int maxSlots(RedstoneProgram program) {
        return 2 * Math.min(program.getChannels().size() + 1,
            DestroyAllConfigs.SERVER.blocks.redstoneProgrammerMaxChannels.get());
    }

    /**
 * Client-side placeholder {@link RedstoneProgram} used during Menu transit — carries just
 * enough state (channels + powered flag) for the Screen UI to render. Real RedstoneProgram
 * lives on server; client-side never writes back directly (sync via C2S packet instead).
*/
    ...
    public static void onPlayerTick(PlayerTickEvent.Post event) {
        ...
        if (player.containerMenu instanceof RedstoneProgrammerMenu menu) {
            ...
            if (oldPower == null || currentPower != oldPower) {
                CatnipServices.NETWORK.sendToClient(player, new RedstoneProgrammerPowerChangedS2CPacket(currentPower));
    ...
    private static final LogicalSide SIDE_SENTINEL = LogicalSide.SERVER;
}
