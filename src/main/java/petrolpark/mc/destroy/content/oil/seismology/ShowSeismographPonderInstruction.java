 ...
 * overlays (row/column/cross/cell) over the grid.
 *
 * <p>Private ctor + public static {@link #add} factory: scenes do
 * {@code SeismographElement element = ShowSeismographPonderInstruction.add(scene, ...);}
 * then call {@code element.highlightRow(scene, ...)} / {@code element.close()} during the
 * scene progression to drive the highlights.</p>
*/
public class ShowSeismographPonderInstruction extends FadeInOutInstruction {
    ...
    public static SeismographElement add(SceneBuilder scene, Pointing direction, Vec3 sceneSpace, Seismograph seismograph) {
...
*/
        ...
        public void render(PonderScene scene, PonderUI screen, GuiGraphics graphics, float partialTicks, float fade) {
            ...
            SeismographItemRenderer.renderSeismograph(ms, graphics.bufferSource(), 0, null, null, seismograph, mc,
                (t, x, y) -> t.render(graphics, (int) x, (int) y), false);
            ms.translate(0f, 0f, 100f);
            for (Outline outline : outlines) {
                outline.texture.render(graphics, outline.x, outline.y,
                    outline.color.getColorObject().setAlpha(1f).scaleAlpha(outline.fade.getValue(partialTicks)));
            }
            ms.popPose();
            ms.popPose();
