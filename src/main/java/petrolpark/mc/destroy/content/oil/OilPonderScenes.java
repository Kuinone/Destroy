...
import petrolpark.mc.destroy.content.oil.seismology.ShowSeismographPonderInstruction;
import petrolpark.mc.destroy.content.oil.seismology.ShowSeismographPonderInstruction.SeismographElement;

/**
 * Ponder scenes for Destroy's oil subsystem — 3 tutorials: {@link #seismometer} (crafting the
 * seismograph/seismometer + TNT-detonation surveying intro), {@link #seismograph} (319 LoC —
 * full nonogram-solving walk-through with row/column/cell highlights driven by
 * {@link ShowSeismographPonderInstruction}), {@link #pumpjack} (pumpjack drilling +
 * CRUDE_OIL fluid pump out).
*/
public class OilPonderScenes {
    ...
    public static void seismometer(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        var cartographyTableSelection = util.select().position(cartographyTablePos);
        scene.idle(10);
        ...
        scene.idle(40);
        scene.world().createItemEntity(util.vector().of(1.5d, 2.2d, 0.75d), util.vector().of(0d, 0.1d, 0.2d),
            DestroyItems.TOUCH_POWDER.asStack());
        scene.idle(80);
    ...
    public static void seismograph(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.configureBasePlate(0, 0, 3);
        ElementLink<WorldSectionElement> firstBasePlate = scene.world().showIndependentSection(
            util.select().fromTo(0, 0, 0, 2, 0, 2), Direction.UP);
        ...
        SeismographElement element = ShowSeismographPonderInstruction.add(scene, Pointing.RIGHT,
            util.vector().topOf(playerPos), seismograph);
        scene.idle(20);
        scene.addKeyframe();
        ...
        for (int i : new int[] { 0, 4, 5 }) {
            for (int j = 0; j < 8; j++) {
                if (j != 3) markSeismograph(scene, seismograph, j, 5,
                    (j >= i && j <= i + 2) ? Mark.GUESSED_TICK : Mark.GUESSED_CROSS);
            }
            scene.idle(40);
        }
        for (int j = 0; j < 8; j++) {
        ...
        for (int i = 3; i >= 0; i--) {
            for (int j = 0; j <= 4; j++) {
                markSeismograph(scene, seismograph, 3, j,
                    (j == i || j == i + 1) ? Mark.GUESSED_TICK : Mark.GUESSED_CROSS);
            }
            scene.idle(30);
        }
        ...
        for (int i : new int[] { 7, 6, 4, 3 }) {
            for (int j = 3; j <= 8; j++) {
                if (j != 5) markSeismograph(scene, seismograph, 3, j,
                    (j == i) ? Mark.GUESSED_TICK : Mark.GUESSED_CROSS);
            }
            scene.idle(30);
        }
        scene.idle(20);
        ...
        scene.overlay().showText(180)
            .text("This text is defined in a language file.")
            .attachKeyFrame();
        ElementLink<WorldSectionElement> grass = scene.world().showIndependentSection(
            util.select().fromTo(0, 0, 3, 2, 1, 14), Direction.UP);
        ElementLink<WorldSectionElement> secondBasePlate = scene.world().showIndependentSection(
            util.select().fromTo(0, 0, 15, 2, 0, 17), Direction.UP);
        scene.idle(20);
        scene.world().modifyEntity(player, e -> e.zo = -1d);
        ...
        for (byte[] numbers : new byte[][] {
            ...
            scene.idle(20);
            scene.effects().emitParticles(VecHelper.getCenterOf(util.grid().at(tick ? 0 : 2, 1, 1)),
                scene.effects().particleEmitterWithinBlockSpace(ParticleTypes.EXPLOSION, Vec3.ZERO), 1f, 1);
            scene.idle(10);
        ...
        scene.world().modifyEntity(player, e -> e.zo = e.position().z);
        scene.idle(20);
    ...
    public static Seismograph newSeismograph(SceneBuilder scene) {
        Seismograph seismograph = new Seismograph();
        scene.addInstruction(s -> seismograph.clear());
        return seismograph;
    }

    public static void markSeismograph(SceneBuilder scene, Seismograph seismograph, int x, int z, Mark mark) {
        scene.addInstruction(s -> seismograph.mark(x, z, mark));
    }

    public static void setSeismographRow(SceneBuilder scene, Seismograph seismograph, int row, byte data) {
    ...
    public static void setSeismographColumn(SceneBuilder scene, Seismograph seismograph, int column, byte data) {
    ...
    public static void pumpjack(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        var pumpjack = util.select().fromTo(1, 1, 3, 3, 2, 3);
        var kinetics = util.select().position(3, 1, 4).add(util.select().fromTo(2, 0, 5, 3, 1, 5));
        var pipes = util.select().fromTo(2, 1, 1, 3, 1, 2);
    ...
    public static ElementLink<EntityElement> addTNTAndPlayer(SceneBuilder scene, SceneBuildingUtil util,
                                                             BlockPos tntPos, BlockPos playerPos) {
        // Add player
        ...
        // Set and then un-set the Player invisible so it doesn't awkwardly jerk when added to the scene
        ...
        scene.effects().emitParticles(VecHelper.getCenterOf(tntPos),
            scene.effects().particleEmitterWithinBlockSpace(ParticleTypes.EXPLOSION_EMITTER, Vec3.ZERO), 1f, 1);
