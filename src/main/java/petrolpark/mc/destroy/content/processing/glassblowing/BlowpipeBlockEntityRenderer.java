...
/**
 * Blowpipe BER — renders the growing/cooling glass bubble at the pipe tip as a sequence of
 * stacked fluid-alpha boxes (one per {@link BlowingShape} in the active recipe). Animates a
 * spline-eased grow phase up to {@link BlowpipeBlockEntity#BLOWING_TIME_PROPORTION}, then
 * a fade-out phase during which the cooling result ItemStack is also rendered through the
 * custom {@link DestroyItemDisplayContexts#BLOWPIPE} display context.
*/
public class BlowpipeBlockEntityRenderer extends SafeBlockEntityRenderer<BlowpipeBlockEntity> {
    ...
    @Override
    protected void renderSafe(BlowpipeBlockEntity blowpipe, float partialTicks, PoseStack ms,
        ...
        ms.pushPose();
        ms.translate(0.5, 0.5, 0.5); // catnip Translate.center() inline
        Direction facing = blowpipe.getBlockState().getValue(BlockStateProperties.FACING);
        poseRotateToFace(ms, facing.getOpposite());
        ms.translate(0f, -2 / 16f, 0.5f);
    ...
    /** Uses
 * {@link Direction#getRotation()} (+Y → direction) followed by an X-axis -90° rotation
 * (+Y → +Z).
*/
    private static void poseRotateToFace(PoseStack ms, Direction direction) {
        ms.mulPose(direction.getRotation());
        ms.mulPose(com.mojang.math.Axis.XP.rotationDegrees(-90f));
    }

    public static void render(GlassblowingRecipe recipe, FluidStack fluid, float progress, PoseStack ms,
        ...
        if (progress > BlowpipeBlockEntity.BLOWING_TIME_PROPORTION) {
            ...
            mc.getItemRenderer().renderStatic(recipe.getResultItem(mc.level.registryAccess()),
                DestroyItemDisplayContexts.BLOWPIPE, light, overlay, ms, buffer, mc.level, 0);
        } else {
        ...
        for (int j = 0; j < shapes; j++) {
            DestroyFluidRenderer.renderFluidBoxWithAlpha(fluid,
                -radii[j] - 1 / 512f, -radii[j] - 1 / 128f, ends[j] - 1 / 512f,
                radii[j] + 1 / 512f, radii[j] + 1 / 128f, ends[j + 1] + 1 / 512f,
                buffer, ms, light, fluidOpacity);
        }
    }
}
