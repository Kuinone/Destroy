...
public class CircuitPunchingBehaviour extends BeltProcessingBehaviour {
    ...
    /**
 * Contract for a block-entity host of a {@link CircuitPunchingBehaviour}. Supplies the actual
 * punch logic + a kinetic-speed accessor (so the behaviour can pause animation when the puncher
 * is unpowered) + a completion callback (for awarding advancements, etc.).
*/
...
*/
    ...
    public void read(CompoundTag tag, HolderLookup.Provider registries, boolean clientPacket) {
        ...
        if (clientPacket) {
            particleItem = ItemStack.parseOptional(registries, tag.getCompound("ParticleItem"));
            spawnParticles();
        }
    ...
    public void write(CompoundTag tag, HolderLookup.Provider registries, boolean clientPacket) {
        ...
        if (clientPacket && !particleItem.isEmpty()) tag.put("ParticleItem", particleItem.save(registries, new CompoundTag()));
