...
/**
 * The "magic" beetroot shoots crop block — randomly grows into one of many
 * {@code *_INFUSED_BEETROOT} variants. Counterpart to vanilla beetroot crop.
 *
 * <p>1.21.1 notes:</p>
*/
public class MagicBeetrootShootsBlock extends BushBlock implements BonemealableBlock {

    public static final MapCodec<MagicBeetrootShootsBlock> CODEC = RecordCodecBuilder.mapCodec(inst -> inst.group(
        propertiesCodec()
    ).apply(inst, MagicBeetrootShootsBlock::new));
    ...
    public void randomTick(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
        if (!level.isAreaLoaded(pos, 1)) return; // prevent loading unloaded chunks when checking neighbor's light
