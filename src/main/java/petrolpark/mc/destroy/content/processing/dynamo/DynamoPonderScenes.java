...
/**
 * Dynamo ponder scenes — 4 story-board methods (arcFurnace / dynamoRedstone / dynamoCharging /
...
public class DynamoPonderScenes {
    ...
    public static void arcFurnace(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(60);
        scene.overlay().showControls(util.vector().blockSurface(furnaceLid, Direction.NORTH), Pointing.RIGHT, 40)
            .rightClick()
            .withItem(DestroyBlocks.CARBON_FIBER_BLOCK.asStack());
        scene.idle(20);
        ...
        scene.overlay().showText(120)
            ...
            .attachKeyFrame();
        scene.idle(20);
        scene.overlay().showControls(util.vector().blockSurface(basin.east(), Direction.EAST), Pointing.RIGHT, 30)
        ...
        scene.idle(10);
        scene.overlay().showControls(util.vector().blockSurface(basin.west(), Direction.WEST), Pointing.LEFT, 30)
    ...
    public static void dynamoRedstone(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world().setKineticSpeed(util.select().position(2, 4, 2), -256); // Reverse the one cog that must spin the other way
        ...
        scene.world().modifyBlockEntityNBT(util.select().position(2, 1, 1), NixieTubeBlockEntity.class,
    ...
    public static void dynamoCharging(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world().modifyBlockEntity(dynamo, DynamoBlockEntity.class, be ->
            be.chargingBehaviour.start(ChargingBehaviour.Mode.BELT,
                util.vector().blockSurface(depot, Direction.UP), 240));
        scene.idle(60);
