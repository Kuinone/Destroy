...
/**
 * Block-Entity Renderer for the Bubble Cap — renders the contained fluid as up to 3 stacked
 * boxes (bottom pipe section + center body + top pipe section) based on the tower's TOP/BOTTOM
...
public class BubbleCapRenderer extends SmartBlockEntityRenderer<BubbleCapBlockEntity> {
    ...
    protected void renderSafe(BubbleCapBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
        ...
        if (!visualTank.isEmpty(partialTicks)) {
            ...
            if (!bottom && bottomLevel > 0f) NeoForgeCatnipServices.FLUID_RENDERER.renderFluidBox(visualTank.getRenderedFluid(), 4 / 16f, 0 / 16f, 4 / 16f, 12 / 16f, 0 / 16f + (bottomLevel * 3 / 16f), 12 / 16f, buffer, ms, light, true, true);
            if (centerLevel > 0f) NeoForgeCatnipServices.FLUID_RENDERER.renderFluidBox(visualTank.getRenderedFluid(), 3 / 16f, 3 / 16f, 3 / 16f, 13 / 16f, 3 / 16f + (centerLevel * 10 / 16f), 13 / 16f, buffer, ms, light, true, true);
            if (!top && topLevel > 0f) NeoForgeCatnipServices.FLUID_RENDERER.renderFluidBox(visualTank.getRenderedFluid(), 4 / 16f, 13 / 16f, 4 / 16f, 12 / 16f, 13 / 16f + (topLevel * 3 / 16f), 12 / 16f, buffer, ms, light, false, true);
        }
    }
