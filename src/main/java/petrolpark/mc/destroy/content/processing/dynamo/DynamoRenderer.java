...
/**
 * Dynamo BER — draws the spinning shaft (via {@link KineticBlockEntityRenderer} parent's
 * rotation-animation path) plus electric-arc lines between the two Dynamo electrodes and the
 * charging target position while {@link DynamoBlockEntity#isRunning()} + {@link
 * DynamoBlockEntity#shouldRenderArcs()}. Arc lines jitter each frame on a 3-segment path
...
public class DynamoRenderer extends KineticBlockEntityRenderer<DynamoBlockEntity> {
    ...
    @Override
    protected void renderSafe(DynamoBlockEntity dynamo, float partialTicks, PoseStack ms,
                              MultiBufferSource buffer, int light, int overlay) {
        super.renderSafe(dynamo, partialTicks, ms, buffer, light, overlay);
        if ((!dynamo.isRunning() && dynamo.soundDuration <= 0) || !dynamo.hasLevel()) return; // getLevel() can't actually be null here
        ...
        Vec3 electrodePos1 = Vec3.atLowerCornerOf(dynamo.getBlockPos())
            .add(northSouthAligned ? 3 / 16f : 8 / 16f, -2 / 16f, northSouthAligned ? 8 / 16f : 3 / 16f);
        Vec3 electrodePos2 = Vec3.atLowerCornerOf(dynamo.getBlockPos())
            .add(northSouthAligned ? 13 / 16f : 8 / 16f, -2 / 16f, northSouthAligned ? 8 / 16f : 13 / 16f);
        ...
        for (boolean whichElectrode : Iterate.trueAndFalse) {
            ...
            PetrolparkCreateClient.OUTLINER.showLine("dynamo_line_0_" + (whichElectrode ? "0" : "1"), electrodePos, firstRandomPos)
                .colored(0xa3f0f7)
                .lineWidth(1 / 64f)
                .disableCull();
            PetrolparkCreateClient.OUTLINER.showLine("dynamo_line_1_" + (whichElectrode ? "0" : "1"), firstRandomPos, secondRandomPos)
                .colored(0xa3f0f7)
                .lineWidth(1 / 64f)
                .disableCull();
            PetrolparkCreateClient.OUTLINER.showLine("dynamo_line_2_" + (whichElectrode ? "0" : "1"), secondRandomPos, targetPos)
                .colored(0xa3f0f7)
                .lineWidth(1 / 64f)
                .disableCull();
        }
    ...
    protected SuperByteBuffer getRotatedModel(DynamoBlockEntity be, BlockState state) {
        return CachedBuffers.partial(state.getValue(DynamoBlock.ARC_FURNACE)
            ? DestroyPartials.ARC_FURNACE_SHAFT
            : DestroyPartials.DYNAMO_SHAFT, state);
    }
