...
/**
 * Animated custom renderer for {@link SwissArmyKnifeItem} — draws the case + 6 sub-tool models,
 * each rotating in/out based on per-entity LerpedFloat chasers (driven by
 * {@code SwissArmyKnifeItem.ClientState.tick()}).
*/
public class SwissArmyKnifeItemRenderer extends CustomRenderedItemModelRenderer {
    ...
    protected void render(ItemStack stack, CustomRenderedItemModel model, PartialItemModelRenderer renderer,
        ...
        if (owner != null) {
            boolean firstPerson = transformType == ItemDisplayContext.FIRST_PERSON_RIGHT_HAND
                || transformType == ItemDisplayContext.FIRST_PERSON_LEFT_HAND;
            ...
            for (RenderedTool tool : RenderedTool.ORDERED_TOOLS) {
                ...
                if (toolAngle != null) {
                    TransformStack.of(ms)
                        ...
                        .rotateZDegrees(179 * (1 - toolAngle.getValue(pt))
                            * (tool == RenderedTool.LOWER_SHEARS ? 1f : -1f))
                        .translate(-5 / 16f, 5 / 16f, 0f);
                }
                itemRenderer.renderStatic(renderedTool, ItemDisplayContext.NONE, light,
                    OverlayTexture.NO_OVERLAY, ms, buffer, mc.level, 0);
                ms.popPose();
            }
            ms.popPose();
    ...
    public static enum RenderedTool {
        CASING(Tool.PICKAXE), // Tool.PICKAXE not used here
        ...
        private static final List<RenderedTool> ORDERED_TOOLS =
            List.of(PICKAXE, AXE, LOWER_SHEARS, UPPER_SHEARS, SHOVEL, HOE);
        ...
        public ItemStack getRenderedItemStack(SwissArmyKnifeItem item) {
            ...
            stack.set(DestroyDataComponents.RENDERED_TOOL, (float) ordinal() / 8f);
            return stack;
        }

        /**
 * Item property getter hook (ItemProperties.register). Reads the
 * {@link DestroyDataComponents#RENDERED_TOOL} DataComponent.
*/
        public static float getItemProperty(ItemStack stack, @Nullable ClientLevel level,
                                            @Nullable LivingEntity entity, int seed) {
            Float f = stack.get(DestroyDataComponents.RENDERED_TOOL);
            return f == null ? 0f : f;
        }
    }
}
