...
public class DestroyVoxelShapes {
    ...
    AGING_BARREL_INTERIOR = shape(0, 0, 0, 16, 16, 16)
    ...
    // pumpjack/ subdir ice breaker. Shape map for the 4 Component enum values of a
    // Pumpjack (FRONT drilling-head, BACK counter-weight, MIDDLE controller, TOP beam).
    public static final VoxelShaper getPumpJackShaper(
            petrolpark.mc.destroy.content.oil.pumpjack.IPumpjackStructuralBlock.Component component) {
        switch (component) {
            case FRONT:
                return shape(4, 0, 0, 12, 8, 12)
                    .forDirectional(net.minecraft.core.Direction.NORTH);
            case BACK:
    ...
    public static VoxelShape agingBarrelClosed(int progress) {
