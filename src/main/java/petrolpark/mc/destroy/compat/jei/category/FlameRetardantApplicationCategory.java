...
/**
 * JEI category for {@link FlameRetardantApplicationRecipe} · displays a Spout dispensing fire-
 * retardant fluid onto an item, yielding a fireproofed variant. When no focused input is given,
 * shows example items (STICK / SHULKER_BOX / IRON_PICKAXE) with an info tooltip explaining the
 * generic fireproofing effect.
*/
public class FlameRetardantApplicationCategory extends PetrolparkRecipeCategory<FlameRetardantApplicationRecipe> {

    /** 1.21 builder pattern
 * doesn't expose static accessors so we mirror the same approach as ReactionCategory.TYPE +
 * GenericReactionCategory.TYPE which already use this pattern).
*/
    public static mezz.jei.api.recipe.RecipeType<? extends net.minecraft.world.item.crafting.RecipeHolder<? extends FlameRetardantApplicationRecipe>> TYPE;
    ...
    public FlameRetardantApplicationCategory(Info<FlameRetardantApplicationRecipe> info, IJeiHelpers helpers) {
        super(info, helpers);
        FlameRetardantApplicationCategory.TYPE = info.recipeType();
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, FlameRetardantApplicationRecipe recipe, IFocusGroup focuses) {
        ...
        builder.addSlot(RecipeIngredientRole.INPUT, 27, 51)
            ...
            .addRichTooltipCallback((v, t) -> {
                if (example) {
                    t.add(DestroyLang.translate("recipe.fireproofing.info").style(ChatFormatting.GOLD).component());
                }
            });
