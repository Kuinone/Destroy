...
/**
 * JEI icon that renders three stacked item stacks at progressively reduced scale.
 * Extends Create's {@link DoubleItemIcon} (which renders 2 stacks); we layer a 3rd at 50% scale
 * with a small offset.
*/
public class TripleItemIcon extends DoubleItemIcon {
    ...
    public void draw(GuiGraphics graphics, int xOffset, int yOffset) {
        if (tertiaryStack == null) tertiaryStack = tertiarySupplier.get();
        ...
        GuiGameElement.of(tertiaryStack).render(graphics);
        ms.popPose();
    }
}
