...
import petrolpark.mc.destroy.compat.jei.tooltip.ReactionTooltipHelper;
...
/**
 * JEI category for {@link ReactionRecipe} · displays a chemistry reaction with its full
 * reactant / catalyst / product / precipitate grid + molar ratios + hover-text paragraphs
 * (reaction name + description + UV/electrons hints when applicable).
*/
public class ReactionCategory<T extends ReactionRecipe> extends HoverableTextCategory<T> {
    ...
    public static RecipeType<? extends RecipeHolder<? extends ReactionRecipe>> TYPE;
    ...
    /** Generate a JEI recipe for every Reaction that opted into JEI display.*/
    static {
    ...
    public Collection<LinesAndActivationAreas> getHoverableTexts(ReactionRecipe recipe) {
        ...
        paragraphs.add(AbstractStackedTextBox.getTextAndActivationAreas(
            Component.translatable(getTranslationKey(recipe)).getString(),
            2, 2, 176, minecraft.screen, minecraft.font, DARK_GRAY_AND_BLUE, false));
        paragraphs.add(AbstractStackedTextBox.getTextAndActivationAreas(
            Component.translatable(getTranslationKey(recipe) + ".description").getString(),
            2, 90, 176, minecraft.screen, minecraft.font, DARK_GRAY_AND_BLUE, false));

        if (reaction.needsUV()) {
            Vector2i pos = getCatalystRenderPosition(0, getNumberOfCatalysts(reaction));
            paragraphs.add(AbstractStackedTextBox.getTextAndActivationAreas(
                DestroyLang.translate("tooltip.reaction.ultraviolet").string(),
                pos.x + 3, pos.y + 4, 100, minecraft.screen, minecraft.font,
                DestroyLang.WHITE_AND_WHITE, false));
        }
        ...
        if (reaction.isHalfReaction()) {
            ...
            paragraphs.add(AbstractStackedTextBox.getTextAndActivationAreas(
                DestroyLang.translate("tooltip.reaction.electrons",
                    DestroyLang.nothingIfOne(reaction.getElectronsTransferred())).string(),
                pos.x + 3, pos.y + 4, 100, minecraft.screen, minecraft.font,
                DestroyLang.WHITE_AND_WHITE, false));
            paragraphs.add(AbstractStackedTextBox.getTextAndActivationAreas(
                DestroyLang.translate("tooltip.reaction.standard_half_cell_potential_hoverable",
                    reaction.getStandardHalfCellPotential()).string(),
                72, 36, 100, minecraft.screen, minecraft.font,
                DestroyLang.WHITE_AND_WHITE, false));
        }
    ...
    private static void tooManyMoleculesWarning(boolean reactants, LegacyReaction reaction) {
        Destroy.LOGGER.warn("Reaction '" + reaction.getFullId() + "' has too many "
            + (reactants ? "reactants" : "products") + " to fit on JEI.");
    }

    public static float getSpeciesWeightForSorting(LegacySpecies s) {
        LegacyAtom atom = Collections.max(s.getAtoms(),
            Comparator.comparing(a -> a.getElement() == LegacyElement.R_GROUP ? a.rGroupNumber : -1));
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, T recipe, IFocusGroup focuses) {
        ...
        Collection<PrecipitateReactionResult> precipitates = reaction.hasResult()
            ? reaction.getResult().getAllPrecipitates()
            : Collections.emptyList();

        float molarRatioMultiplier = 1f;

        Optional<IItemReactant> focusItemReactant = reaction.getItemReactants().stream()
            .filter(reactant -> focuses.getItemStackFocuses(RecipeIngredientRole.INPUT)
                .anyMatch(focus -> reactant.isItemValid(focus.getTypedValue().getIngredient())))
            .findFirst();

        Optional<PrecipitateReactionResult> focusPrecipitate = Optional.ofNullable(reaction.getResult())
            .map(reactionResult -> reactionResult.getAllPrecipitates().stream()
                .filter(result -> focuses.getItemStackFocuses(RecipeIngredientRole.OUTPUT)
                    .anyMatch(focus -> result.getPrecipitate().is(focus.getTypedValue().getIngredient().getItem())))
        ...
        for (LegacySpecies reactant : reaction.getReactants().stream()
                .sorted(Comparator.comparing(ReactionCategory::getSpeciesWeightForSorting)).toList()) {
            if (i >= 6) continue;
        ...
        for (IItemReactant itemReactant : reaction.getItemReactants()) {
            ...
            if (!itemReactant.isCatalyst()) {
                ...
                IRecipeSlotBuilder slotBuilder = builder.addSlot(
                        reaction.displayAsReversible() ? RecipeIngredientRole.CATALYST : RecipeIngredientRole.INPUT,
                        pos.x, pos.y)
        ...
        for (LegacySpecies product : reaction.getProducts().stream()
                .sorted(Comparator.comparing(ReactionCategory::getSpeciesWeightForSorting)).toList()) {
            if (j >= 6) continue;
            builder.addSlot(RecipeIngredientRole.OUTPUT,
                    productsXOffset + (19 * (j % l)), productYOffset + (j / l) * 19)
        ...
        for (PrecipitateReactionResult precipitate : precipitates) {
            if (j >= 6) continue;
            IRecipeSlotBuilder slotBuilder = builder.addSlot(
                    reaction.displayAsReversible() ? RecipeIngredientRole.CATALYST : RecipeIngredientRole.OUTPUT,
                    productsXOffset + (19 * (j % l)), productYOffset + (j / l) * 19)
        ...
        int m = 0;
        if (reaction.needsUV()) m++;

        for (LegacySpecies catalyst : reaction.getOrders().keySet().stream()
                .sorted(Comparator.comparing(ReactionCategory::getSpeciesWeightForSorting)).toList()) {
    ...
    protected int getNumberOfReactants(LegacyReaction reaction) {
        return reaction.getReactants().size()
            + reaction.getItemReactants().stream().filter(ir -> !ir.isCatalyst()).toList().size()
            + (reaction.isHalfReaction() ? 1 : 0);
    }
    ...
    public void draw(T recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics graphics, double mouseX, double mouseY) {
        ...
        (recipe.getReaction().displayAsReversible()
            ? PetrolparkGuiTexture.JEI_EQUILIBRIUM_ARROW
            : AllGuiTextures.JEI_ARROW).render(graphics, yOffset + 37, 46);
    }
        ...
        return new IDrawable() {
            ...
            public void draw(GuiGraphics graphics, int xOffset, int yOffset) {
                ...
                graphics.drawString(fontRenderer, s,
                    (int) ((xOffset + getWidth()) / scale - fontRenderer.width(s) + 1),
                    (int) ((yOffset + getHeight()) / scale - 7),
                    0xFFFFFF, true);
