    ...
    private static class Listener implements ResourceManagerReloadListener {
        ...
        public void onResourceManagerReload(ResourceManager resourceManager) {
            ...
            for (String namespace : resourceManager.getNamespaces()) {
                ResourceLocation fileLocation = ResourceLocation.fromNamespaceAndPath(namespace, "lang/destroy_compat/name_lists/" + minecraft.getLanguageManager().getSelected() + ".json");
                Optional<Resource> resource = resourceManager.getResource(fileLocation);
                if (resource.isEmpty()) { // Default to American English
                    fileLocation = ResourceLocation.fromNamespaceAndPath(namespace, "lang/destroy_compat/name_lists/en_us.json");
                ...
                if (!resource.isEmpty()) {
                    try (InputStream inputStream = resource.get().open()) {
                        ...
                        for (Entry<String, JsonElement> entry : jsonObject.entrySet()) {
                            ResourceLocation nameListRl = ResourceLocation.fromNamespaceAndPath(namespace, entry.getKey());
