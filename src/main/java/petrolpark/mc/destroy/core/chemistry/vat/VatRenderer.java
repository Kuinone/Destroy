...
/**
 * BER for the {@link VatControllerBlockEntity} controller block — draws the 6 Vat-side cells'
 * visual overlays (PIPE nozzle / BAROMETER dial / THERMOMETER / blocked interfaces / VENT bars)
 * + floating inventory items + liquid + gas fluid boxes filling the Vat's internal volume.
*/
public class VatRenderer extends SafeBlockEntityRenderer<VatControllerBlockEntity> {
    ...
    protected void renderSafe(VatControllerBlockEntity controller, float partialTicks, PoseStack ms, MultiBufferSource bufferSource, int light, int overlay) {
        ...
        if (itemCount != 0) {
            ...
            for (int slot = 0; slot < inv.getSlots(); slot++) {
                ...
                for (int item = 0; item < stack.getCount(); item++) {
                    Vec3 itemPosition = center.add(VecHelper.rotate(south.scale(offset * (nthItem - 1)), angle, Axis.Y));

                    ms.pushPose();
                    if (fluidLevel > 0) ms.translate(0, (Mth.sin(AnimationTickHolder.getRenderTime(controller.getLevel()) / 12f + angle) + 1.5f) * 1 / 32f, 0);
        ...
        if (!gasStack.isEmpty()) {
            NeoForgeCatnipServices.FLUID_RENDERER.renderFluidBox(gasStack,
