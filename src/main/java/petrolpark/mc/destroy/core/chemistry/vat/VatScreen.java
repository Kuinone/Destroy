...
/**
 * Main Vat GUI — molecule list (scrollable, filterable, view-toggled BOTH/LIQUID/GAS) + selected
 * molecule 3D render + name/info scrollable text + rotating 3D Vat controller model in corner.
*/
    ...
    protected void updateMoleculeList() {
            ...
            default: {
                if (fluid != null) {
                    ...
                    if (DestroyFluids.isMixture(fluid)) {
                        mixture = ReadOnlyMixture.readNBT(ClientMixture::new,
        ...
        for (LegacySpecies molecule : mixture.getContents(false)) {
            ...
            if (filter == null || filter.getValue().isEmpty()
                || molecule.getName(false).getString().toUpperCase().indexOf(search) > -1
                || molecule.getName(true).getString().toUpperCase().indexOf(search) > -1
                || molecule.getSerlializedMolecularFormula(false).toUpperCase().indexOf(search) > -1) {
