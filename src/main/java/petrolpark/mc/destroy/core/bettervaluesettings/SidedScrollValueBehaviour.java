...
/**
 * Per-side ScrollValueBehaviour — stores 6 independent {@link Direction#ordinal} indexed values
 * (one per face) instead of one global value. Used by Dynamite (excavation radius per face) and
 * any future block needing direction-keyed scroll settings.
*/
public class SidedScrollValueBehaviour extends BlockEntityBehaviour implements BetterValueSettingsBehaviour {
    ...
    /**
 * Label the scroll box on one face of the Block with the <em>opposite</em> side of the Block
 * (e.g. the north side is labelled "South").
*/
    ...
    // 1.21 Create added HolderLookup.Provider as the FIRST arg to both clipboard methods.
    @Override
    public boolean readFromClipboard(net.minecraft.core.HolderLookup.Provider registries,
                                     CompoundTag tag, Player player, Direction side, boolean simulate) {
    ...
    @Override
    public boolean writeToClipboard(net.minecraft.core.HolderLookup.Provider registries,
                                    CompoundTag tag, Direction side) {
        if (!acceptsValueSettings()) return false;
    ...
    public void onShortInteract(Player player, InteractionHand hand, Direction side, BlockHitResult hitResult) {
        ...
        // useWithoutItem. FakePlayer fallback path: trigger the block's no-item right-click so
        // automation (Deployer etc.) can still operate the value box without needing an item.
        if (player instanceof FakePlayer) {
            blockEntity.getBlockState().useWithoutItem(getWorld(), player, hitResult);
        }
    }
