...
/**
 * Destroy Mixed-Explosive chemistry descriptor: per-explosive-item table of five {@link ExplosiveProperty
 * ExplosiveProperties} (ENERGY / OXYGEN_BALANCE / TEMPERATURE / BRISANCE / SENSITIVITY) with
 * associated condition thresholds (CAN_EXPLODE / DROPS_EXPERIENCE / etc.). Loaded from
 * {@code data/<ns>/destroy_compat/explosive_items/<item>.json} resource files via
...
public class ExplosiveProperties extends EnumMap<ExplosiveProperties.ExplosiveProperty, ExplosiveProperties.ExplosivePropertiesEntry> {
    ...
    /**
 * Reload listener for {@code data/<ns>/destroy_compat/explosive_items/<item>.json} data files.
